...
@st.cache_data
def CSV_data(trades_link: str, position_link: str, sectors_link: str):
    columns = ['Symbol', 'TradeDate', 'Quantity', 'TradePrice', 'DefaultPositions']
    trades = pd.read_csv(trades_link, parse_dates=['TradeDate'], date_format='%m/%d/%y')
    portfolio = get_treemap_data(link_pos=position_link, link_sec=sectors_link)
    return trades, portfolio

trades, portfolio = CSV_data(link_trades, link_positions, link_sectors)
...
def plot_candlestick(df: pd.DataFrame, trades: pd.DataFrame, tick):
    ...
    #tr_min = df_trades_365["DefaultPositions"].min()
    #tr_max = df_trades_365["DefaultPositions"].max()
    ...
    fig.add_trace(go.Scatter(
        x=df_trades_ticker[df_trades_ticker["Quantity"] > 0]["TradeDate"],
        y=df_trades_ticker[df_trades_ticker["Quantity"] > 0]["TradePrice"],
        mode="markers",
    ...
    fig.add_trace(
        go.Scatter(x=df.index,
                y=df['1wma'],
                line = dict(color = 'blue', dash='dot'), name = '1-week MA',
                hovertemplate="<b>%{x} :</b>  "+"<b> $%{y:.2f}<extra></extra>"
                ),
                row=1,
                col=1
    ...
    fig.add_trace(
        go.Scatter(x=df.index,
                y=df['1mma'],
                line = dict(color = 'magenta', dash='dot'), name = '1-month MA',
                hovertemplate="<b>%{x} :</b>  "+"<b> $%{y:.2f}<extra></extra>"
                ),
                row=1,
                col=1
    ...
    fig.update_layout(
        ...
        xaxis=dict(title='Date', side='bottom', 
                range=[datetime.strftime(date.today()-timedelta(days=365), "%Y-%m-%d"), 
                        datetime.strftime(date.today()+timedelta(days=3), "%Y-%m-%d")],
                        type="date", 
...
st.markdown('<span style="font-size:9pt; color: grey;">Last Update: 04/11/2025</span>', unsafe_allow_html=True)
st.divider()
